...
class RealDataExperiment:
    ...
    def __init__(
        ...
        # get response
        self._y = pd.get_dummies(self._data[response_col_name], drop_first=True).astype(
            int
        )
    ...
    def get_params(self):
        lib_lin_sol = list(self._iterative_model.intercept_) + list(
            self._iterative_model.coef_[0]
        )
...
def plot_mse_v_num_predictors(
    PATHNAME, response_col_nam, dataset_name, penalty, drop_these=[]
):
    ...
    for num_cols in range(1, max_num_cols):
        ...
        real_data_experiment = RealDataExperiment(
            response_col_name=response_col_name,
            num_cols=num_cols,
            data_pathname=PATHNAME,
            num_levels=2,
            drop_these=drop_these,
            penalty=penalty,
        )
    ...
    plt.plot(
        x_vals,
        gamma_avg_magnitudes,
        label='Avg Magnitude of Analytic Est.',
        linewidth=4,
        alpha=0.6,
    )
    plt.plot(
        x_vals,
        iterative_beta_avg_magnitudes,
        label='Avg Magnitude of iterative MLE',
        color="green",
        linewidth=4,
        alpha=0.6,
    )
    ...
    plt.legend()
    path_name = '/Users/joshdavis/Desktop/STAT RESEARCH/Analytical-Solutions-Logistic-Regression/Saved Plots/'
    plt.savefig(
        path_name + dataset_name + " Real Data. RidgePenalty=" + str(penalty) + "_.png",
        dpi=300,
    )


def prediction_accuracy_compariosn(
    num_trials,
    ...
    real_data_experimenrt = RealDataExperiment(
        response_col_name=response_col_name,
...
def beta_standard_error_comparison(
    PATHNAME,
    response_col_nam,
    dataset_name,
    penalty,
    ...
    for i in range(num_beta_samples):
        sampled_gamma, sampled_iterative_beta = sample_betas(
            gamma_predicted_success_prob,
            beta_MLE_predicted_success_prob,
            all_unique_rows,
            count_per_row,
            penalty=0,
        )
    ...
    analytic_beta_monte_carlo_standard_error = np.var(
        analytic_betas, axis=0, ddof=1
    ) ** (0.5)

    iterative_beta_monte_carlo_standard_error = np.var(
        iterative_monte_Caro_betas, axis=0, ddof=1
    ) ** (0.5)
    ...
    asymptotic_standard_error = np.array([XWX[i][i] for i in range(len(XWX))]) ** (0.5)
    ...
    ax1.errorbar(
        y=[0 + i for i in range(len(analytic_monte_carlo_means))],
        x=gamma,
        xerr=standard_error_multiplier * analytic_beta_monte_carlo_standard_error,
        fmt='o',
        capsize=6,
        color="blue",
        label='Analytic MC SE',
    )

    # iterative error bars
    ax1.errorbar(
        y=[0.2 + i for i in range(len(iterative_monte_carlo_means))],
        x=iterative_beta,
        xerr=standard_error_multiplier * iterative_beta_monte_carlo_standard_error,
        fmt='o',
        linewidth=2,
        capsize=5,
        color="orange",
        label='Iterative MC SE',
    )

    # asymptotic error bars
    ax1.errorbar(
        y=range(len(analytic_monte_carlo_means)),
        x=gamma,
        xerr=standard_error_multiplier * asymptotic_standard_error,
        fmt='o',
        capsize=5,
        color="darkblue",
        linewidth=4,
        alpha=0.3,
        label='Analytic Asymptotic SE',
    )

    # asymptotic error bars
    ax1.errorbar(
        y=[0.2 + i for i in range(len(analytic_monte_carlo_means))],
        x=iterative_beta,
        xerr=standard_error_multiplier * asymptotic_standard_error,
        fmt='o',
        capsize=5,
        ...
        linewidth=4,
        alpha=0.3,
        label='Iterative Asymptotic SE',
    )

    names = ["Intercept"] + list(real_data_experiment.get_column_names())

    title = (
        "Beta Confidence Intervals: Estimate plus / minus "
        + str(standard_error_multiplier)
        + " SE"
    )

    ax1.set_title(title)

    fig.suptitle(dataset_name + " Model Comparison", y=0.95, fontsize=16)

    plt.figtext(
        0.5,
        0.92,
        "MonteCarlo Sample Size: " + str(num_beta_samples),
        ha='center',
        fontsize=12,
    )
    ...
    table_array = np.column_stack(
        (
            gamma,
            analytic_beta_monte_carlo_standard_error,
            iterative_beta,
            iterative_beta_monte_carlo_standard_error,
        )
    )
    ...
    # Create table
    table = ax2.table(
        cellText=table_array[::-1],
        loc='center',
        cellLoc='center',
        colLabels=["Analytic Value", "Analytic SE", "Iterative Value", "Iterative SE"],
        rowLabels=names,
    )
    bbox = [0.13, 0.03, 1, 1]
    table.scale(0.9, 3)  # Increase size by scaling

    ax2.set_title("Estimated Regression Coefficients with MC Standad Error", y=1.03)
    # Adjusting font size
    ...
    ax2.add_table(table)
    path_name = '/Users/joshdavis/Desktop/STAT RESEARCH/Analytical-Solutions-Logistic-Regression/Saved Plots/'
    plt.savefig(path_name + dataset_name + " Standard Errors.png", dpi=300)
...
def sample_betas(
    ...
    for row, gamma_prob, MLE_prob, num_samples in zip(
        design,
        gamma_predicted_success_prob,
        beta_MLE_predicted_success_prob,
        count_per_row,
    ):
        y_gamma += np.random.binomial(n=1, p=gamma_prob, size=num_samples).tolist()
        y_iterative += np.random.binomial(n=1, p=MLE_prob, size=num_samples).tolist()
        X += [row] * num_samples
    ...
    iterative_beta = list(iterative_model.intercept_) + list(iterative_model.coef_[0])
...
PATHNAME = '/Users/joshdavis/Desktop/Analytic Logsitic Regression RESEARCH/Analytical-Solutions-Logistic-Regression/datsets/breastcancer_data.csv'
...
num_beta_samples = 1000
beta_standard_error_comparison(
    PATHNAME=PATHNAME,
    response_col_nam=response_col_name,
    dataset_name=dataset_name,
    num_beta_samples=num_beta_samples,
    drop_these=drop_these,
    penalty=penalty,
)
