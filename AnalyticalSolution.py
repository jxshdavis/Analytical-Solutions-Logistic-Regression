...
class AnalyticalSolution:
    ...
    def __init__(self, x, sample_size_info, drop_under_count, lamb=0, nudge=10 ** (-5), y=np.array([0]), large_sample_rows=None):
        """
    ...
    def transform_response(self):
        ...
        stored_all_unique_rows = ((unq[:, None] & (1 << np.arange(num_bits)[::-1]))
                                  > 0).astype(int)

        ones_column = np.ones((stored_all_unique_rows.shape[0], 1), dtype=int)

        self._all_unique_rows = np.hstack(
            (ones_column, stored_all_unique_rows))
        ...
        for drop_under_count in self._drop_under_count:
            ...
            remaining_batch_integers = np.setdiff1d(
                batch_integers, nums_to_delete)
            ...
            sample_weights = new_cnt * new_success_frequencies * \
                (1-new_success_frequencies)
            ...
            if self._lambda != 0:
                ...
                linear_model = LinearRegression().fit(new_unq, z, sample_weight=sample_weights)
